...
if ($membercode ne 'ad' && $membercode ne 'smo' && $inmembmod ne 'yes') {
    ...
    $post.="[br][br]\[color=$fonthighlight\]\[b\]-=-=-=-=- 以下内容由 \[i\]$postermembername\[\/i\] 在 \[i\]$time1\[\/i\] 时添加 -=-=-=-=-\[\/b\]\[\/color\]<br>".$inpost;
    print FILE "$postermembername\t$topictitle\t$postipaddress\t$showemoticons\t$showsignature\t$postdate\t$post\t$posticon\t$water\n";}
    else{
    print FILE "$postline\n";
    }
    }
    ...
    &winunlock($filetoopen) if ($OS_USED eq "Nt");
    &mischeader("续写贴子");
    $gopage = int(($posttoget-2)/$maxtopics)*$maxtopics;
    if ($refreshurl == 1) { 
