...
$onlinetitle = $onlineview == 1 ? "[<a href=$thisprog?action=onlineview&forum=$inforum&topic=$intopic&start=$instart&show=$inshow&man=$man1><font color=$titlefontcolor>关闭详细列表</font></a>]" : "[<a href=$thisprog?action=onlineview&forum=$inforum&topic=$intopic&start=$instart&show=$inshow&man=$man1><font color=$titlefontcolor>显示详细列表</font></a>]";
...
if ($treeview eq "yes") { $viewstyle="&nbsp;<a href=topic.cgi?forum=$inforum&topic=$intopic&changemode=1&show=$inshow&man=$man1><img src=$imagesurl/images/flatview.gif width=40 height=12 border=0 alt=平板显示贴子></a>"; }
                   else { $viewstyle="&nbsp;<a href=topic.cgi?forum=$inforum&topic=$intopic&changemode=1&show=$inshow&man=$man1><img src=$imagesurl/images/treeview.gif width=40 height=12 border=0 alt=树形显示贴子></a>"; }
...
if ($numberofpages > 1) {
	...
	my $beginpage = $currentpage == 1 ? "<font color=$fonthighlight face=webdings>9</font>" : qq~<a href=$thisprog?forum=$inforum&topic=$intopic&start=0&show=$inshow&man=$man1 title="首 页" ><font face=webdings>9</font></a>~;
	my $endpage = $currentpage == $numberofpages ? "<font color=$fonthighlight face=webdings>:</font>" : qq~<a href=$thisprog?forum=$inforum&topic=$intopic&start=$endstart&show=$inshow&man=$man1 title="尾 页" ><font face=webdings>:</font></a>~;

	my $uppage = $currentpage - 1;
	my $nextpage = $currentpage + 1;
	my $upstart = $instart - $maxtopics;
	my $nextstart = $instart + $maxtopics;
	my $showup = $uppage < 1 ? "<font color=$fonthighlight face=webdings>7</font>" : qq~<a href=$thisprog?forum=$inforum&topic=$intopic&start=$upstart&show=$inshow&man=$man1 title="第$uppage页"><font face=webdings>7</font></a>~;
	my $shownext = $nextpage > $numberofpages ? "<font color=$fonthighlight face=webdings>8</font>" : qq~<a href=$thisprog?forum=$inforum&topic=$intopic&start=$nextstart&show=$inshow&man=$man1 title="第$nextpage页"><font face=webdings>8</font></a>~;
	...
	my $showupstep = $upsteppage < 1 ? "" : qq~<a href=$thisprog?forum=$inforum&topic=$intopic&start=$upstepstart&show=$inshow&man=$man1 class=hb title="第$upsteppage页">←</a> ~;
	my $shownextstep = $nextsteppage > $numberofpages ? "" : qq~<a href=$thisprog?forum=$inforum&topic=$intopic&start=$nextstepstart&show=$inshow&man=$man1 class=hb title="第$nextsteppage页">→</a> ~;
	...
	for (my $i = $upsteppage + 1; $i < $nextsteppage; $i++)
	{
		...
		$pages .= $i == $currentpage ? "<font color=$fonthighlight><b>$i</b></font> " : qq~<a href=$thisprog?forum=$inforum&topic=$intopic&start=$currentstart&show=$inshow&man=$man1 class=hb>$i</a> ~;
...
<td width=350 valign=bottom>$newthreadbutton$newreplybutton$newpollbutton</td>
...
<td align=right> <a href=$thisprog?forum=$inforum&topic=$intopic&show=$inshow&start=$instart&man=$man1&max=yes><img src=$imagesurl/images/showall.gif border=0 width=14 alt=不分页显示此帖></a>&nbsp;$savefile$reporticon$favicon$printpageicon$pagpageicon$bookmarkpage$sendtofriendicon $fontsizeselect&nbsp;</td></tr></table></td><td bgcolor=$tablebordercolor width=1 height=24></td></tr></table>
<table cellspacing=0 cellpadding=0 width=$tablewidth bgcolor=$tablebordercolor align=center><tr><td height=1></td></tr></table>
~;
...
postnum.innerHTML="<form method=post action=addpost.cgi><input name=id type=hidden value="+num+"><input name=forum type=hidden value=$inforum><input name=topic type=hidden value=$intopic><textarea rows=6 name=inpost cols=35></textarea><p></p><input type=submit value=续写该文></form>";
}
...
#    next if ($membername eq "");
    ...
    if (($mymembercode eq "ad")||($mymembercode eq 'smo')||($myinmembmod eq "yes")||(($usereditpost ne "no")&&(lc($inmembername) eq lc($membername{$membername}))))  { $editgraphic    = qq~<a href=editpost.cgi?action=edit&forum=$inforum&topic=$intopic&postno=$editpostnumber title=编辑这个贴子><img src=$imagesurl/images/edit.gif border=0 width=16 align=absmiddle>编辑</a>　~; } else { $editgraphic =""; }
    ...
    if ((lc($inmembername) eq lc($membername))&&($inmembername ne "客人")){
    ...
    if ($man eq '') {
        ...
        $postmanbuttom ="<a href=$thisprog?forum=$inforum&topic=$intopic&show=$inshow><img src=$imagesurl/images/find.gif border=0 width=16 align=absmiddle>取消只看我</a>　";
    }
    ...
    if (($signature{$membername}) and ($showsignature eq 'yes') and ($nodispsign eq "no"||$mynodispsign eq "no") and ($posticon !~/<br>/i)) {
        if ($signature{$membername} ne "") { $post = qq($post<BR></td><td width=16></td></tr><tr><td></td><td valign=bottom><BR><BR><BR><BR>$postcopyright<img src=$imagesurl/images/none.gif width=1 height=3><BR><img src=$imagesurl/images/signature.gif height=12><br><img src=$imagesurl/images/none.gif width=1 height=5><BR>$signature{$membername}<BR>); }
